...
lemma norm_mul_le_mul {a b : ℂ} {x y : ℝ} (ha : ‖a‖ ≤ x) (hb : ‖b‖ ≤ y) (hx : 0 ≤ x) :
    ‖a * b‖ ≤ x * y :=
  (norm_mul a b).trans_le (mul_le_mul ha hb (norm_nonneg _) hx)
...
theorem φ₀_bound (z : ℍ) (hz : 1 / 2 < z.im) :
  ...
  exact (norm_mul_le_mul (norm_g_le z hz.le) (g_div_Δ_bound z hz)
    (mul_nonneg B_g_pos.le (Real.exp_pos _).le)).trans_eq (by ring)
...
theorem φ₄'_bound (z : ℍ) (hz : 1 / 2 < z.im) :
  ...
  rw [hfact, C_φ₄']
  exact (norm_mul_le_mul (norm_E₄_le z hz.le) (E₄_div_Δ_bound z hz) B_E₄_pos.le).trans_eq (by ring)
