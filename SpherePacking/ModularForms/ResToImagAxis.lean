...
theorem ResToImagAxis.Pos.add {F G : ℍ → ℂ} (hF : ResToImagAxis.Pos F)
    (hG : ResToImagAxis.Pos G) : ResToImagAxis.Pos (F + G) :=
  ⟨Real.add hF.1 hG.1, fun t ht ↦ by
...
theorem ResToImagAxis.Pos.mul {F G : ℍ → ℂ} (hF : ResToImagAxis.Pos F)
    (hG : ResToImagAxis.Pos G) : ResToImagAxis.Pos (F * G) :=
  ⟨Real.mul hF.1 hG.1, fun t ht ↦ by
    have hFreal := hF.1 t ht; have hGreal := hG.1 t ht
    have hFpos := hF.2 t ht; have hGpos := hG.2 t ht
    simp only [Function.resToImagAxis, ResToImagAxis, ht, ↓reduceDIte]
      at hFreal hGreal hFpos hGpos ⊢
    simp [hFreal, hGreal, mul_pos hFpos hGpos]⟩

theorem ResToImagAxis.Pos.hmul {F : ℍ → ℂ} {c : ℝ} (hF : ResToImagAxis.Pos F)
    (hc : 0 < c) : ResToImagAxis.Pos (fun z => c * F z) :=
  ⟨Real.hmul hF.1, fun t ht ↦ by
...
theorem ResToImagAxis.EventuallyPos.add {F G : ℍ → ℂ}
  ...
  refine ⟨Real.add hF.1 hG.1, max tF tG, by positivity, fun t ht ↦ ?_⟩
  have htpos : 0 < t := (lt_max_iff.mpr (.inl hF0)).trans_le ht
  have hFpos_t := hFpos t ((le_max_left _ _).trans ht)
  have hGpos_t := hGpos t ((le_max_right _ _).trans ht)
  simp only [Function.resToImagAxis_apply, ResToImagAxis, htpos, ↓reduceDIte] at hFpos_t hGpos_t
  simp only [Function.resToImagAxis_apply, ResToImagAxis, htpos, ↓reduceDIte, Pi.add_apply, add_re]
...
theorem ResToImagAxis.EventuallyPos.mul {F G : ℍ → ℂ}
  ...
  obtain ⟨tF, hF0, hFpos⟩ := hF.2
  obtain ⟨tG, hG0, hGpos⟩ := hG.2
  refine ⟨Real.mul hF.1 hG.1, max tF tG, by positivity, fun t ht ↦ ?_⟩
  have htpos : 0 < t := (lt_max_iff.mpr (.inl hF0)).trans_le ht
...
theorem ResToImagAxis.EventuallyPos.hmul {F : ℍ → ℂ} {c : ℝ}
  ...
  refine ⟨Real.hmul hF.1, t₀, hF0, fun t ht ↦ ?_⟩
