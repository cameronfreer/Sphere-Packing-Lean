...
lemma f₄_sq_mul_eq (z : ℍ) (hg_z : theta_g z = 0) :
  ...
  have h_Af₂_eq : A * f₂ z + B * f₄ z = 0 := by
    simp only [theta_g, Pi.add_apply, Pi.mul_apply, Pi.smul_apply, smul_eq_mul] at hg_z
    convert hg_z using 1
  -- Af₂ = -Bf₄
...
lemma f₂_eq_zero : f₂ = 0 := by
  ...
  suffices hf₄ : f₄ = 0 by
    ...
    simp only [theta_h, Pi.add_apply, Pi.mul_apply, Pi.pow_apply, hf₄z,
               mul_zero, add_zero, zero_pow (by norm_num : 2 ≠ 0)] at hz
...
lemma f₄_eq_zero : f₄ = 0 := by
  ...
  simp only [theta_h, Pi.add_apply, Pi.mul_apply, Pi.pow_apply, hf₂z,
             zero_pow (by norm_num : 2 ≠ 0), zero_mul, add_zero, zero_add] at hz
  exact sq_eq_zero_iff.mp hz
...
theorem serre_D_H₂ :
  ...
  simp only [f₂, Pi.zero_apply, Pi.sub_apply, Pi.smul_apply, Pi.mul_apply,
             Pi.add_apply, smul_eq_mul, sub_eq_zero] at this
  convert this using 1
  ring
...
theorem serre_D_H₃ : serre_D 2 H₃ = fun z => (1/6 : ℂ) * (H₂ z ^ 2 - H₄ z ^ 2) := by
  funext z
  have := congrFun f₃_eq_zero z
  simp only [f₃, Pi.zero_apply, Pi.sub_apply, Pi.smul_apply, Pi.pow_apply,
             smul_eq_mul, sub_eq_zero] at this
  exact this
...
theorem serre_D_H₄ :
  ...
  funext z
  have := congrFun f₄_eq_zero z
  simp only [f₄, Pi.zero_apply, Pi.add_apply, Pi.smul_apply, Pi.mul_apply,
             smul_eq_mul] at this
  -- this : serre_D 2 H₄ z + 1/6 * (H₄ z * (2 * H₂ z + H₄ z)) = 0
  have h : (1/6 : ℂ) * (H₄ z * (2 * H₂ z + H₄ z)) =
      (1/6 : ℂ) * (2 * H₂ z * H₄ z + H₄ z ^ 2) := by ring
  rw [h, add_eq_zero_iff_eq_neg] at this
  convert this using 1
  ring
