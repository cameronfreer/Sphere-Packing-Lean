...
lemma cc (f : ℤ → ℂ) (hc : CauchySeq fun N : ℕ => ∑ m ∈ Finset.Icc (-N : ℤ) N, f m)
  ...
  · rw [trex f n.natAbs] at H3
    · simp only [dist_eq_norm, Nat.cast_natAbs, Int.cast_abs, Int.cast_eq, neg_sub,
        Int.cast_sub, Int.cast_one, gt_iff_lt] at *
      have h1 : |n| = n := by
        simp
        linarith
      simp_rw [h1] at H3
...
lemma CauchySeq_Icc_iff_CauchySeq_Ico (f : ℤ → ℂ) (hs : ∀ n, f n = f (-n))
  ...
  have hff : Tendsto (fun n : ℕ => 2 * ‖f n‖) atTop (𝓝 0) := by
    rw [Metric.tendsto_atTop] at *
    simp only [gt_iff_lt, ge_iff_le, dist_eq_norm, sub_zero, zero_mul,
      norm_ofNat, norm_mul, Real.norm_ofNat, norm_norm] at *
    intro ε hε
    have hf3 := h1 ε hε
    obtain ⟨N, hN⟩ := hf3
    use N.natAbs
